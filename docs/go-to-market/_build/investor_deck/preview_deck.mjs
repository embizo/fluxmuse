...
function slideHTML(s) {
  ...
  s.els.forEach((e, i) => {
    ...
    if (e.type === 'pic') {
      ...
      const rows = e.rows.map((r) => `<tr data-rh="${r.h * PX}" data-txt="${esc(r.cells.map((c)=>c.paras.map((p)=>p.runs.map((x)=>x.text).join('')).join(' ')).join(' | ')).replace(/"/g,'')}" style="height:${r.h * PX}px">${r.cells.map((c) => `<td style="background:#${c.fill || 'FFFFFF'};padding:${0.04 * PX}px ${0.12 * PX}px;border-bottom:1px solid #E3E6EA;vertical-align:middle;overflow-wrap:break-word">${runsHTML(c.paras)}</td>`).join('')}</tr>`).join('');
  ...
  for (const s of L.slides) {
    ...
    const res = await b.evaluate(`(() => {
      ...
      document.querySelectorAll('.tb').forEach((d) => {
        ...
        if (over > 2) out.push(d.dataset.name + ' #' + d.dataset.i + ' [' + inner.innerText.slice(0, 40) + '] overflows by ' + (over / 96).toFixed(2) + ' in');
        if (wide > 2) out.push(d.dataset.name + ' #' + d.dataset.i + ' too wide by ' + (wide / 96).toFixed(2) + ' in');
      });
      document.querySelectorAll('.tbl').forEach((t) => {
        const grow = t.getBoundingClientRect().height - Number(t.dataset.h);
        if (grow > 3) {
          out.push('table grows by ' + (grow / 96).toFixed(2) + ' in (rows expand)');
          t.querySelectorAll('tr').forEach((tr, ri) => {
            const g2 = tr.getBoundingClientRect().height - Number(tr.dataset.rh);
            if (g2 > 2) out.push('  table row ' + ri + ' +' + (g2 / 96).toFixed(2) + ' in: ' + tr.dataset.txt.slice(0, 80));
          });
        }
      });
      return out;
